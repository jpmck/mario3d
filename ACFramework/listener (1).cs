...
namespace ACFramework
{
    ...
    class cListener
    {
        public static readonly float CRITTERTURNSPEEDSLOW = 0.1f * (float)Math.PI; //Slow Radians per second to turn. Try 0.5*PI 
        public static readonly float CRITTERTURNSPEEDFAST = 0.5f * (float)Math.PI; //Fast Radians per second to turn. Try 2.0*PI 
    ...
    class cListenerHopper : cListener
    {
        ...
        public bool _childsmode;
        float timeCLastPressed;
        ...
        public override void listen(float dt, cCritter pcritter)
        {
            ...
            bool up = Framework.keydev[vk.Up];
            bool down = Framework.keydev[vk.Down];
            bool pageup = Framework.keydev[vk.ControlLeft];
            bool pagedown = Framework.keydev[vk.PageDown];
            ...
            if (left)
            {
                ...
                if (!_faceleft && _faceright)
                {
                    pcritter.Sprite.rotate((float)Math.PI);
                    _faceleft = true;
                    _faceright = false;
                }
            }
            ...
            {
                //Pulse upwards
                Framework.snd.play(Sound.Jump); // makes the jump sound when jumping
            ...
            if (c && (pcritter.Age>timeCLastPressed+1))
            {
                if (_childsmode)
                {
                    _childsmode = false;
                    pcritter.Sprite = new cSpriteQuake(ModelsMD2.Luigi);
                    Framework.snd.play(Sound.PowerOn);
                }
                else
                {
                    _childsmode = true;
                    pcritter.Sprite = new cSpriteQuake(ModelsMD2.Mario);
                    Framework.snd.play(Sound.PowerOff);
                }
                timeCLastPressed = pcritter.Age;
                if (_faceleft)
                    pcritter.Sprite.rotate((float)Math.PI);
            }
    ...
    class cListenerViewerRide : cListener //Assume 3D 
    {
        ...
        public static readonly cVector3 OFFSETDIR = new cVector3(-1.1f, 15.0f, 2.0f);
        public static readonly float PLAYERLOOKAHEAD = 0.0f;
